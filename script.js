...
if (document.getElementById('preview')) {
  ...
  addTextBtn.addEventListener('click', () => {
    ...
    box.addEventListener('blur', () => {
      if (!box.innerText || box.innerText.trim() === '') { box.innerText = DEFAULT_TEXT; box.classList.add('placeholder'); }
    });
  ...
  document.addEventListener('pointermove', e => {
    ...
    if (targetBox && targetBox.dataset.dragPending === 'true') {
      ...
      const moved = Math.sqrt(Math.pow(e.clientX - sx,2) + Math.pow(e.clientY - sy,2));
      if (moved > 6) {
  ...
  document.addEventListener('pointerup', e => {
    document.querySelectorAll('.text-box').forEach(b => { delete b.dataset.dragPending; delete b.dataset.startX; delete b.dataset.startY; });
  });

  function setActiveBox(el) { clearActiveBox(); el.classList.add('active'); activeTextBox = el; textToolbar.style.display = 'block'; }
  ...
  function draw() {
    ...
    for (let i=0;i<4;i++){
      ...
      if(!photos[i]){ ctx.fillStyle='#eee'; ctx.fillRect(0,y,PREVIEW_W,slotH); }
        ...
        const img = photos[i]; const imgRatio = img.width/img.height; const slotRatio = PREVIEW_W/slotH;
        let sw,sh,sx,sy;
    ...
    if(selectedFrame && selectedFrame.imgObj){ try{ ctx.drawImage(selectedFrame.imgObj,0,0,PREVIEW_W,PREVIEW_H);}catch(e){} }
  ...
  exportBtn.addEventListener('click', ()=>{
    ...
    const octx = out.getContext('2d'); octx.fillStyle='#fff'; octx.fillRect(0,0,EXPORT_W,EXPORT_H);

    const slotH = EXPORT_H/4;
    for(let i=0;i<4;i++){
      if(!photos[i]) continue;
      const img = photos[i]; const imgRatio=img.width/img.height; const slotRatio=EXPORT_W/slotH;
      let sw,sh,sx,sy;
      if(imgRatio>slotRatio){ sh=img.height; sw=img.height*slotRatio; sx=(img.width-sw)/2; sy=0; }
      else { sw=img.width; sh=img.width/slotRatio; sx=0; sy=(img.height-sh)/2; }
      const dy=i*slotH;
